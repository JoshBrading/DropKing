...
bool ray_intersects_plane(const Ray& ray, const Vector3 plane_point, const Vector3 plane_normal, float* out_distance)
{
    ...
    const float denom = Vector3DotProduct(ray.direction, plane_normal);
    if (std::fabs(denom) < 0.0001f) // If almost parallel
    ...
    const float t = Vector3DotProduct(Vector3Subtract(plane_point, ray.position), plane_normal) / denom;
